...
Options:
  ...
  --build-id ID                Immutable id (default: pump-v<version>-<12-char HEAD>)
...
png="target/ui-screenshots/pump/pump-default-912x684.png"
[[ -f "${png}" ]] || { echo "default screenshot was not produced" >&2; exit 1; }
cp "${png}" "${release_dir}/pump-default-912x684.png"
...
audit_zip() {
  ...
  if [[ "${format}" == clap ]]; then /usr/bin/nm -gU "${contents}/MacOS/pump" | grep -q _clap_entry || { echo "CLAP ZIP entrypoint missing" >&2; exit 1; }; else
    for symbol in _GetPluginFactory _bundleEntry _bundleExit; do /usr/bin/nm -gU "${contents}/MacOS/pump" | grep -q "${symbol}" || { echo "VST3 ZIP symbol ${symbol} missing" >&2; exit 1; }; done
  fi
}
