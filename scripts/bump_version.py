#!/usr/bin/env python3
"""Update Pump's package and lockfile version for one release bump."""
...
def bump_package_version(manifest_path: Path, lock_path: Path, new_version: str) -> str:
    ...
    if lock_matches != 1:
        raise ValueError("Cargo.lock Pump package entry is missing or ambiguous")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"bumped Pump from v{previous} to v{sys.argv[1]}")
    return 0
