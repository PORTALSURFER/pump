...
- Incoming-waveform rendering is a low-contrast symmetric envelope behind the editable curve, nodes, playhead, and interaction feedback. Backward phase discontinuities larger than a small floating-point epsilon start a new capture generation, including host loops or seeks shorter than half a Pump cycle. Zero-frame processing callbacks do not publish or refresh capture data, so stale input still ages out during parameter-only or keepalive traffic. Default CI passes 218 tests, VST3 CI passes 244 tests, and screenshot validation passes at 315x211, 420x282, 525x352, and 630x423.
