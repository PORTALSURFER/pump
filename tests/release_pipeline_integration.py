#!/usr/bin/env python3
"""Validate the Pump nightly artifact and publisher-integration contracts."""
...
TEST_RELEASE_TOKEN = "pump-integration-release-token"
TEST_OIDC_REQUEST_TOKEN = "pump-integration-oidc-request-token"
TEST_ATTESTATION_TOKEN = "pump-integration-attestation-token"
TEST_NOTARY_ID = "00000000-0000-4000-8000-000000000000"
CAPABILITY_PATH = "/plugins/api/v1/products/pump/releases"
...
def _require_shared_source(manifest: dict[str, Any], source_sha: str, label: str) -> None:
    ...
    require(source.get("repository") == "PORTALSURFER/pump", f"{label} repository is invalid")
...
def _load_macos_preflight(
    ...
    *,
    ...
    require(manifest.get("product") == "pump", "macOS preflight product is invalid")
    ...
    require(clap_name == f"pump-v{package_version}-macos.clap.zip", "macOS preflight CLAP archive name is invalid")
    require(vst3_name == f"pump-v{package_version}-macos.vst3.zip", "macOS preflight VST3 archive name is invalid")
    ...
    require(len(list(root.glob("pump-default-*.png"))) == 1, "macOS preflight must have exactly one screenshot")
...
def _load_windows_artifact(
    ...
    *,
    ...
    require(manifest.get("product") == "pump", "Windows artifact product is invalid")
...
class ApiMock:
    ...
    def _release_upload_prefix(self) -> str:
        ...
        return f"/plugins/api/v1/products/pump/release-uploads/{build_id}"
...
def _assembled_release(args: argparse.Namespace) -> Iterator[tuple[dict[str, Any], Path]]:
    ...
    with tempfile.TemporaryDirectory(prefix="pump-release-integration-", dir=scratch_parent) as directory:
        scratch = Path(directory)
        mac_clap_destination = scratch / f"pump-v{args.publication_version}-macos.clap.zip"
        mac_vst3_destination = scratch / f"pump-v{args.publication_version}-macos.vst3.zip"
...
def run_publisher_integration(args: argparse.Namespace) -> None:
    ...
    with _assembled_release(args) as (manifest, scratch):
        ...
        manifest_bytes = release_helper.canonical_json(manifest)
        audience = f"{api_endpoint}/plugins/api/v1/products/pump/release-attestations/sha256/{hashlib.sha256(manifest_bytes).hexdigest()}"
